...
// Pulls safe display data out of an image content block. Inline images are
// limited to inert raster formats: SVG/HTML content must never become a
// clickable data URL in the privileged desktop renderer. Remote URLs are
// returned as links only; the UI deliberately does not auto-load them, which
// avoids turning a transcript record into an invisible tracking request.
...
function safeExternalImageUrl(value) {
  try {
    ...
    // External images are never fetched by Fama itself. The user may choose
    // to open a link in their browser, but only over an authenticated,
    // encrypted transport.
...
function mediaFromImageBlock(block) {
  ...
  if (source.type === 'base64' && source.data) {
    ...
    const mediaType = String(source.media_type || 'image/png').toLowerCase();
    if (!SAFE_INLINE_IMAGE_TYPES.has(mediaType)) return null;
    return { mediaType, data: source.data };
  }
  if (source.type === 'url' && source.url) {
    const externalUrl = safeExternalImageUrl(source.url);
