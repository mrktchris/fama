...
const { eventsFromBuzzHeartbeat } = require('./parse-buzz-heartbeat');
...
class SseEventFeed {
  ...
  publish(event) {
    ...
    this._backlog.push({ id, payload, bytes });
  ...
  subscribe(client, { replay = true, lastEventId = null } = {}) {
    this._clients.add(client);
    if (replay) this.replay(client, lastEventId);
    return () => this._clients.delete(client);
  }

  replay(client, lastEventId = null) {
    const after = Number(lastEventId);
    for (const item of this._backlog) {
      if (Number.isFinite(after) && Number(item.id) <= after) continue;
      client.write(item.payload);
    }
  }
...
class LiveActivityIngest {
  constructor(options) {
    ...
    this._parseBuzzHeartbeat = options.parseBuzzHeartbeat || eventsFromBuzzHeartbeat;
  ...
  scan(now = this._now()) {
    ...
    for (const filePath of this._scanBuzzHeartbeats()) seen.add(filePath);
  ...
  _publishRecords(records, project, provider, context) {
    const parse = provider === 'claude'
      ? this._parseClaude
      : provider === 'codex'
        ? this._parseCodex
        : this._parseBuzzHeartbeat;
    for (const record of records) {
      ...
      for (const event of parse(record, context)) {
        const sessionId = provider === 'buzz' ? `${project.id}:${event.sessionId}` : event.sessionId;
        this.feed.publish(Object.assign(event, { sessionId, provider, projectId: project.id, projectName: project.name }));
      }
    }
  }
  ...
  _scanBuzzHeartbeats() {
    const seen = new Set();
    for (const project of this.projects) {
      const filePath = this._path.join(project.cwd, '.fama', 'agent-heartbeats.jsonl');
      let stat;
      try {
        stat = this._fs.statSync(filePath);
      } catch {
      ...
      seen.add(filePath);
      this._attach(filePath, stat.size, (records) => this._publishRecords(records, project, 'buzz'));
    }
    return seen;
  }
}
