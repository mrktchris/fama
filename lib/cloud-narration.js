...
const REWRITE_PRICE_PER_TOKEN = { prompt: 0.00000015, completion: 0.0000006 };
...
class CloudNarration {
  constructor(options) {
    ...
    this._config = {
      ...
      model: saved.OPENAI_TTS_MODEL || this._env.OPENAI_TTS_MODEL || 'tts-1-hd',
      voice: saved.OPENAI_TTS_VOICE || this._env.OPENAI_TTS_VOICE || 'alloy',
      rewriteModel: saved.OPENAI_REWRITE_MODEL || this._env.OPENAI_REWRITE_MODEL || 'gpt-4o-mini',
  ...
  updateSettings(body) {
    ...
    this._config = {
      ...
      model: nextText('model'),
      voice: nextText('voice'),
      rewriteModel: nextText('rewriteModel'),
  ...
  _recordUsage({ ttsChars, ttsModel, rewritePromptTokens, rewriteCompletionTokens }) {
    ...
    if (rewritePromptTokens || rewriteCompletionTokens) {
      cost +=
        (rewritePromptTokens || 0) * REWRITE_PRICE_PER_TOKEN.prompt +
        (rewriteCompletionTokens || 0) * REWRITE_PRICE_PER_TOKEN.completion;
