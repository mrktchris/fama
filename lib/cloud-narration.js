...
const ESTIMATED_CHARACTERS_PER_WORD = 5.5;
const MIN_NARRATION_WORDS = 6;
const TTS_MODEL_CATALOG = Object.freeze([
  Object.freeze({ id: 'gpt-4o-mini-tts', note: 'recommended, supports style control', voiceStyleSupported: true }),
  Object.freeze({ id: 'tts-1', note: '$15/1M chars, no style control', voiceStyleSupported: false }),
  Object.freeze({ id: 'tts-1-hd', note: '$30/1M chars, tested consistently slowest', voiceStyleSupported: false }),
]);
const TTS_MODELS = new Set(TTS_MODEL_CATALOG.map((model) => model.id));
const TTS_MODEL_BY_ID = new Map(TTS_MODEL_CATALOG.map((model) => [model.id, model]));
const TTS_VOICES = Object.freeze(['alloy', 'echo', 'fable', 'onyx', 'nova', 'shimmer']);
const REWRITE_MODELS = new Set(['gpt-4o-mini']);
// Checked against OpenAI's official catalog and model pages on 2026-08-11.
// tts-1 and tts-1-hd are billed per character. gpt-4o-mini-tts is listed in
// the catalog without a deprecation badge and its detail page labels it the
// default; its token-billed per-character rate is necessarily an estimate.
...
function narrationPreset(seconds) {
  ...
  const words = Math.max(MIN_NARRATION_WORDS, Math.round(normalized * NARRATION_WORDS_PER_SECOND));
...
class CloudNarration {
  ...
  publicConfig() {
    ...
    return Object.freeze({
      ...
      voiceStyleSupported: TTS_MODEL_BY_ID.get(config.model).voiceStyleSupported,
      models: TTS_MODEL_CATALOG.map((model) => ({
        id: model.id,
        label: `${model.id} · ${model.note}`,
        voiceStyleSupported: model.voiceStyleSupported,
      })),
      voices: [...TTS_VOICES],
      narrationEstimate: {
        wordsPerSecond: NARRATION_WORDS_PER_SECOND,
        charactersPerWord: ESTIMATED_CHARACTERS_PER_WORD,
        minWords: MIN_NARRATION_WORDS,
      },
      pricing: {
